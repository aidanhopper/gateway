...
// Dir mounts a static directory, file, or SPA serve mount programmatically and returns created route names.
func Dir(ctx context.Context, client GatewayClient, opts DirOptions) ([]string, error) {
	if !client.ConfirmPublicSiteExposure(opts.Yes, fmt.Sprintf("Static Directory %s -> %s", opts.Mount, opts.LocalPath)) {
		return nil, nil
	}
	...
	parsedDomain, path := ParseMount(opts.Mount)
	domainVal := opts.Domain
	if domainVal == "" {
		domainVal = parsedDomain
	}

	absDir, err := filepath.Abs(opts.LocalPath)
	if err != nil {
		return nil, fmt.Errorf("invalid directory path %q: %w", opts.LocalPath, err)
	}
	...
	prefixToStrip := opts.StripPrefix
	if prefixToStrip == "" && !opts.NoStripPrefix && path != "/" && path != "" && !isFile {
		prefixToStrip = path
	}
	...
	if isHTTPS {
		...
		lName := opts.ListenerName
		if lName == "" {
			lName = fmt.Sprintf("serve-https-%s", strings.TrimPrefix(lAddr, ":"))
		}

		useACME := opts.ACME
		if domainVal != "" && !useACME && opts.CertFile == "" {
		...
		var tlsSpec *api.TLSConfigSpec
		if useACME || domainVal != "" || opts.CertFile != "" {
			tlsSpec = &api.TLSConfigSpec{Auto: useACME}
			if domainVal != "" {
				tlsSpec.Domains = []string{domainVal}
			}
			if opts.CertFile != "" {
				if data, e := os.ReadFile(opts.CertFile); e == nil {
					tlsSpec.Cert = string(data)
				}
			}
			if opts.KeyFile != "" {
				if data, e := os.ReadFile(opts.KeyFile); e == nil {
		...
		if err := client.CreateRoute(ctx, routeSpec); err != nil {
			return nil, fmt.Errorf("failed to create HTTPS static route: %w", err)
		}
		...
		fmt.Printf("[SUCCESS] Sharing %s HTTPS %s -> %s\n", modeStr, displayTarget, absDir)
		fmt.Printf("  ├── Listener: %s\n", lAddr)
		if opts.TTL > 0 {
			fmt.Printf("  ├── Mount:    %s\n", routeName)
			fmt.Printf("  └── TTL:      %v (auto-expires)\n", opts.TTL)
		...
		if err := client.CreateRoute(ctx, routeSpec); err != nil {
			return nil, fmt.Errorf("failed to create HTTP static route: %w", err)
		}
		...
		fmt.Printf("[SUCCESS] Sharing %s HTTP %s -> %s\n", modeStr, displayTarget, absDir)
		fmt.Printf("  ├── Listener: %s\n", lAddr)
		if opts.TTL > 0 {
			fmt.Printf("  ├── Mount:    %s\n", routeName)
			fmt.Printf("  └── TTL:      %v (auto-expires)\n", opts.TTL)
